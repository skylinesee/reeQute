...
# Command: Generate verification code
@bot.command()
async def verify(ctx, username: str):
    """Generate and send a verification code to a user"""
    if not username:
        await ctx.send("Please provide a username to verify.")
        return
    ...
    # Try to find and send DM to the user
    try:
        user = None
        for guild in bot.guilds:
        ...
        await ctx.send(f"Verification code sent to {username}.")
    except Exception as e:
        await ctx.send(f"Error sending verification code: {e}")
...
async def send_verification_dm(username, code):
    try:
        ...
        if '#' in username:
            ...
            for guild in bot.guilds:
                for member in guild.members:
                    if member.name.lower() == name.lower() and str(member.discriminator) == discriminator:
                        user = member
